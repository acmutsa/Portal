...
const VisibilityDropdown = ({ columns, onColumnChange }: VisibilityProps) => {
	...
	const [visibleColumns, setVisibleColumns] = useState(
		columns.filter((col) => col?.default ?? true).map((col) => col.id)
	);

	const onChangeHandler = (curVisibleColumns: string[]) => {
		onColumnChange({
			...baseColumnEntries,
			...Object.fromEntries(curVisibleColumns.map((id) => [id, true])),
		});
		...
		>
			<div>
				...
				>
					...
					<HiOutlineSelector className="ml-1.5 -mr-1 mt-0.5 py-[1px] h-5 w-5" aria-hidden="true" />
				</Listbox.Button>
			</div>
			<Listbox.Options className="absolute right-0 z-10 mt-2 min-w-[10rem] origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-xl ring-2 ring-black ring-opacity-5 focus:outline-none">
